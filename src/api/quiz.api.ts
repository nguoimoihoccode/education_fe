...
export const getPublicQuizzes = async (params?: {
  ...
  const response = await apiClient.get("/quizzes/public", {
    params,
    ...CACHE_PROFILES.DYNAMIC,
  });
  return normalizeCollectionPage<Quiz>(response.data, "quizzes");
...
export const startQuizSession = async (
  ...
  if (quizOfflineMode) {
    return offlineQuizProvider.startQuizSession(
      quizId,
      dto as {
...
export const completeQuizSession = async (
  ...
  const response = await apiClient.post(
    `/quizzes/sessions/${sessionId}/complete`,
  );
  return normalizeQuizSessionObject(response.data as QuizSession);
};
...
export const getQuizSessions = async (
  ...
  const response = await apiClient.get(`/quizzes/${quizId}/sessions`, {
    params,
    ...CACHE_PROFILES.DYNAMIC,
  });
  const normalized = normalizeCollectionPage<QuizSession>(
    response.data,
    "sessions",
  );
  return {
    ...normalized,
    items: normalized.items.map((session) =>
      normalizeQuizSessionObject(session),
    ),
  };
};

export const getAllQuizSessions = async (params?: {
  page?: number;
  limit?: number;
}): Promise<PaginatedQuizSessionResponse> => {
  const response = await apiClient.get("/quizzes/sessions", {
    params,
    ...CACHE_PROFILES.DYNAMIC,
  });
  const normalized = normalizeCollectionPage<QuizSession>(
...
export const getQuizStats = async (): Promise<QuizStats> => {
  ...
  return normalizeQuizStats(response.data);
};

export const getQuizStatsByTopic = async (
  topic: string,
): Promise<TopicStats> => {
  const response = await apiClient.get(
    `/quizzes/stats/topic/${topic}`,
    CACHE_PROFILES.USER,
  );
  return normalizeTopicQuizStats(response.data, topic) as TopicStats;
};
...
export const getQuizHistory = async (params?: {
  ...
  const normalized = normalizeCollectionPage<QuizHistoryItem>(
    response.data,
    "sessions",
  );
      ...
      (
        ...
        totalAnswers:
          ...
          (item.correctAnswers ?? 0) +
            ((item as { wrongAnswers?: number }).wrongAnswers ?? 0),
...
export const getWrongAnswers = async (
  ...
  if (sessionId) {
    const response = await apiClient.get(
      `/quizzes/sessions/${sessionId}/wrong`,
      CACHE_PROFILES.NO_CACHE,
    );
    return normalizeWrongAnswers(response.data);
  }
  const response = await apiClient.get(
    "/quizzes/wrong-answers",
    CACHE_PROFILES.NO_CACHE,
  );
  return normalizeWrongAnswers(response.data);
};
